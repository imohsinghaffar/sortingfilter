...
const CartDetails = () => {
  const { cartData } = useContext(CartDataContext); // Access cart data from context
  const [cartQuantity, setCartQuantity] = useState(1);

  const deliveryCharges = 45;

  // const const [quantityUpdate, setQuantityUpdate] = useState()
  ...
  const subtotal = cartData.reduce((sum, item) => sum + (item.price * cartQuantity), 0);
  const total = subtotal + deliveryCharges

  function increament() {
    if (cartQuantity >= 1) {
      let cartQuantityInc = cartQuantity + 1;
      setCartQuantity(cartQuantityInc);
    }
  }

  function decreament() {
    if (cartQuantity > 1) {
      let cartQuantityDec = cartQuantity - 1;
      setCartQuantity(cartQuantityDec);
    }
  }
...
//           return {
//             ...item,
//             quantity: newQuantity >= 1 ? newQuantity : 1 // Prevent going below 1
//           };
//         }
//         return item;
//       });
        ...
        <div className="w-full max-w-7xl px-4 md:px-5 lg-6 mx-auto">
          ...
          <div>
            {cartData.length === 0 ? (
              ...
              cartData.map(({ id, title, image, price, category }) => (
